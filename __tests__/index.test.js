...
const multipleSkip = "node src/index.js samples/A.md samples/B.md samples/C.md samples/D.md -s"
const skipList = `\n✅  Done adding references to 2 files:
...
describe("Linkify your markdown files", () => {
	it("should process single file with or without options", () => run(single, singleDone))

	it("should not process a single file if it is empty", () => run(singleEmpty, emptyMsg))

	it("should process multiple files", () => run(multiple, multipleDone))

	it("should give list of skipped files when multiple files are the input", () => run(multipleSkip, skipList))

	it("should process the whole directory", () => run(directory, dirSkip))

	it("skip processing the directory if it is empty", () => run(emptyDir, wasEmpty))
})
