...
const messages = {
	...
	single: file => log(cyan(`\n✅  Done adding references to your file ${yellow(file)}\n`)),
	// For multiple files
	multiple: (files, skippedFiles) => {
		log(cyan(`\n✅  Done adding references to ${files.length} ${files.length > 1 ? "files:" : "file: "}\n`))
	...
	invalidType: () =>
		log(red(`\nThe provided file is not a ${yellow("markdown")} file. Type ${yellow("linkify --help")} for help. \n`)),
	emptyDir: data => log(cyan(`\nHmm... seems like the directory ${yellow(data.input[0])} is empty.\n`)),
}
...
const processMultipleFiles = (files, options = {}) => {
	...
	files.forEach(file => {
		...
		isEmpty = linkify(options.isDir ? `${options.name}/${file}` : file, getOptions())
...
const processFilesInDir = data => {
	...
	const { processedFiles, skippedFiles } = processMultipleFiles(files, { isDir: true, name: data.input[0] })
...
const isSingleFile = data.input.length === 1 && (!data.flags.d || !data.flags.dir)

const hasMultipleFiles = (!data.flags.d || !data.flags.dir) && data.input.length > 1
